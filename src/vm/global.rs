...
use crate::vm::vmalloc::{MAX_CHUNK_SIZE, Kalloc};
...
impl Galloc {
    pub fn new(pool: &mut PagePool) -> Self {
        let small_pool_start = pool.palloc().expect("Could not initalize GlobalAlloc small pool");
        Galloc {
            pool,
    ...
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        ...
        if num_pages == 0 {
            match (*self.small_pool.get()).alloc(layout.size()) {
                ...
                Err(e) => { panic!("Small allocation failed {:?}", e) }
            }
        } else {
    ...
    unsafe fn dealloc(&self, ptr: *mut u8, layout: Layout) {
        ...
        if num_pages == 0 {
            ...
            match (*self.pool).pfree_plural(ptr as *mut usize, num_pages) {
                Ok(_) => {},
                Err(e) => { panic!("Page deallocation failed {:?}", e)}
            }
        }
