...
/// Performs the hart local initialziation of the PLIC (setting
/// priority threshold mostly). Should be run only after global_init
/// has been run
pub fn hart_local_init() {
    unsafe {
        PLIC.assume_init_ref().lock().hart_local_init();
    }
}
